...
/*void radio_transmit(int n, int e, int s, int w) {
  ...
  Serial.println(info[0], BIN);
  Serial.println(info[1], BIN);
  Serial.println(info[2], BIN);
  Serial.println(info[3], BIN);
  Serial.println();
  //unsigned long more_info = 80;
  ...
  if (role == role_ping_out) {
    ...
    radio.openWritingPipe(pipes[0]);
    byte arr[4] = {5, 2, 3, 6};
    memcpy(transmit, arr, 4);
    bool ok = radio.write(transmit, 4);
